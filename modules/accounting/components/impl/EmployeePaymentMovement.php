...
class EmployeePaymentMovement extends BaseMovement
{
    ...
    {
        try{
            if ($modelInstance instanceof EmployeePayment) {
                if ($action == "update" && $modelInstance->status == "closed") {
                            ...
                            foreach ($amounts[$aca->attrib] as $amount) {
                                ...
                                if ($amount > 0) {
                                    ...
                                    if (!$aca->is_debit) {
                                        ...
                                        foreach ($modelInstance->employeePaymentItems as $pItem) {
                                            if (!array_key_exists($pItem->employee_payment_item_id, $itemsPassed) && $pItem->payment_method_id == $aca->attrib) {
                                                if (!is_null($pItem->money_box_account_id)) {
                                                    $account = $pItem->moneyBoxAccount->account;
                                                    $itemsPassed[$pItem->employee_payment_item_id] = $pItem;
                                                    break;
                                    ...
                                    if ($aca->is_debit) {
                                        $item->account_id = ($modelInstance->employee->account ? $modelInstance->employee->account->account_id : $account->account_id);
                                        $item->debit = $amount;
                                        $item->credit = 0;
                                    } else {
                    ...
                    if( !( $account_movement_id = $countMov->createMovement(Yii::t('app', 'Employee Payment') . " - " . $modelInstance->description . " - " . $modelInstance->employee->fullName . ' .Fecha del comprobante: ' . $modelInstance->date,
