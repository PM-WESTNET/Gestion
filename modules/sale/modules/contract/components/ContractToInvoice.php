...
class ContractToInvoice
{
    ...
    public function invoice($company, $bill_type_id, $customer_id, $period, $includePlan = true, $bill_observation = '')
    {
        ...
        try{
            ...
            if ($company && $bill_type_id && $customer_id) {
                ...
                //Agrego Planes, Items y descuentos a la factura
                $this->addItemsToBillThroughContracts($bill, $contracts, $includePlan, $customerActiveDiscount, $period, $default_unit_id);
    ...
    private function addItemsToBillThroughContracts($bill, $contracts, $includePlan, $customerActiveDiscount, $period, $default_unit_id)
    {
        foreach ($contracts as $contract_value) {
            ...
            $this->getPeriods($periods, $next, $nextPeriod, $contractStart);

            // Verifica que el plan tenga item a facturar, en caso de no tener agrego los Planes
            $this->addPlansToContract($contract->contractDetailsValidForToday, $includePlan, $periods, $customerActiveDiscount, $next,
            ...
            $this->addItemsToBill($bill, $products_to_invoice, $contract, $includePlan, $period, $contractStart, $next, $default_unit_id);

            // Itero en los descuentos aplicados al cliente.
            $this->addDiscountsTobill($bill, $customerActiveDiscount);
        }
    }
    ...
    private function getPeriods(&$periods, &$next, &$nextPeriod, $contractStart)
    {
        ...
        $contractStartYearAndMonth = $contractStart->format('Ym');
        $contractStartDay = $contractStart->format('d');
        ...
        if ($this->invoice_day_for_next_month != 0 && ( $current_month == $contractStartYearAndMonth)  &&
            $this->invoice_day_for_next_month <= $contractStartDay
        ){
            $next = true;
        ...
        foreach($contractDetails as $contractDetail) {
            if($contractDetail->product->type=='plan' && $includePlan) {
                if (!$contractDetail->isAddedForInvoice($periods)){
                    ...
                    //Solicito el precio activo en funcion del contrato de cliente (aplica reglas de negocio)
                    $amount = $this->getContractDetailPrice($contractDetail);
                    ...
                    $pti = $this->createProductToInvoice([
                        ...
                        'amount' => $amount,
                        'status' => 'active',
                        'period' => $next ? $nextPeriodDate : $periodDate,
    ...
    private function addItemsToBill($bill, $products_to_invoice, $contract, $includePlan, $period, $contractStart, $next, $default_unit_id)
    {
        foreach($products_to_invoice as $pti) {
            ...
            $unit_final_price = $this->calculateUnitFinalPrice($pti, $unit_net_price_with_discount);
    ...
    private function calculateUnitFinalPrice($product, $unit_net_price_with_discount)
    {
        if($product->contractDetail) {
            $unit_final_price = $unit_net_price_with_discount + $product->contractDetail->product->calculateTaxes($unit_net_price_with_discount);
        } else {
    ...
    private function getContracts($company_id, $customer_id, $bill_type_id, $period, $includePlan)
    {
        ...
        return $contractSearch->searchForInvoice([], true, $includePlan)->all();
    }
    ...
    /**
     * Devuelve el precio del plan que se debe facturar. Teniendo
     * en cuenta la antiguedad del contrato. Si es un contrato reciente y se cambio el plan masivamente,
     * devuelve el valor del plan anterior, de lo contrario si solo cambio de precio el plan devuelve el precio
     * al momento de comenzar el contrato. Para el resto devuelve el precio actual del plan actual
     * @param ContractDetail $contractDetail
     * @return float|mixed
     * @throws \yii\base\InvalidConfigException
     */
    private function getContractDetailPrice($contractDetail)
    {
        $newCustomerParam = (integer)Config::getValue('months-without-increase');

        $contractStartDate = $contractDetail->contract->from_date;

        $promotionDate = strtotime(Yii::$app->formatter->asDate($contractStartDate, 'yyyy-MM-dd')) + (86400 *30 * $newCustomerParam);

        if (date('m') <= date('m', $promotionDate)) {
            $toDateOld= (new DateTime($contractDetail->from_date))->modify('-1 day');
            $oldPlan= $contractDetail
                ->contract
                ->getContractDetails()
                ->innerJoin('product p', 'p.product_id=contract_detail.product_id')
                ->andWhere(['p.type' => 'plan', 'contract_detail.to_date' => $toDateOld->format('Y-m-d')])
                ->one();
            if ($oldPlan){

                $activePrice = ProductPrice::find()
                    ->where('exp_timestamp >= :now',['now'=>$toDateOld->getTimestamp()])
                    ->andWhere(['product_id' => $oldPlan->product_id])
                    ->orderBy(['product_price_id' => SORT_DESC])
                    ->one();

                if ($activePrice) {
                    return $activePrice->net_price;
                }
            }else {
                $activePrice = ProductPrice::find()
                    ->where('exp_timestamp >= :now',['now'=>strtotime(Yii::$app->formatter->asDate($contractStartDate, 'yyyy-MM-dd'))])
                    ->andWhere(['product_id' => $contractDetail->product_id])
                    ->orderBy(['product_price_id' => SORT_DESC])
                    ->one();
