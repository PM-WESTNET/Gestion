...
class m180514_152616_mobile_push_table extends Migration
{
    public function safeUp()
    {
        $this->execute(
            "CREATE TABLE IF NOT EXISTS `mobile_push` (
                  `mobile_push_id` INT NOT NULL AUTO_INCREMENT,
                  `title` VARCHAR(45) NOT NULL,
                  `content` VARCHAR(45) NOT NULL,
                  `status` ENUM('draft','pending', 'sended') NOT NULL,
                  `send_timestamp` INT NOT NULL,
                  `created_at` INT NOT NULL,
                  PRIMARY KEY (`mobile_push_id`))
                ENGINE = InnoDB");
        ...
        $this->addColumn('user_app', 'player_id', 'VARCHAR(255) NULL');
    }

    public function safeDown()
    {
        $this->dropColumn('user_app', 'player_id');
        $this->dropTable('mobile_push_has_user_app');
        $this->dropTable('mobile_push');
    }
