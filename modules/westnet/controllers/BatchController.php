    ...
    public function actionPlanToCustomerAssign()
    {
        ...
        if (Yii::$app->request->isAjax && Yii::$app->request->isPost) {
            ...
            foreach ( $query->all() as $key => $value) {
                $contract = Contract::findOne($value['contract_id']);
                $actualContractDetail = $contract->getLastContractDetailByType('plan');

                // Si los productos son distintos creo un nuevo detalle
                if($actualContractDetail->product_id != $new_product_id &&  $actualContractDetail->status != Contract::STATUS_DRAFT) {

                    // Cambio la fecha de fin y el estado en el viejo para poder crear el log
                    $actualContractDetail->updateAttributes(['to_date' => (new \DateTime('now'))->modify('-1 day')->format('Y-m-d'), 'status' => ContractDetail::STATUS_LOW]);
                    $actualContractDetail->createLog();
                    ...
                    //NO corro validaciones, al ser el vendor_id requerido, no me permite el salvado. De poner un vendor_id en el nuevo contract detail, será incluido en la liquidación de este vendedor, no siendo ése el comportamiento esperado, ya que estaria liquidando contract details que realmente no ha vendido.
                    $newContractDetail->save(false);
                }
