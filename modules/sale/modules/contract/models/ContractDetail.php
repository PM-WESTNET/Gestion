...
class ContractDetail extends ActiveRecord
{
  ...
  /*  public function behaviors()
    ...
    {
        if($this->product_id != $oldContractDetail->product_id ||
            $this->discount_id != $oldContractDetail->discount_id ||
            $this->from_date != $oldContractDetail->from_date ||
            ($this->to_date  ? $this->to_date : 'Tiempo Indeterminado' ) != ($oldContractDetail->to_date ? $oldContractDetail->to_date : 'Tiempo Indeterminado' )||
...
//        $items = ContractDetail::find()->with('contract', 'product')->join('natural left join', 'vendor_liquidation_item')->joinWith('contract')->where('vendor_liquidation_item.vendor_liquidation_id IS NULL')->andWhere('contract.customer_id>22000')->all();
     ...
     * Actualiza el vendedor del contrato y del contract_detail
     */
    public static function updateDetaillVendorAndContractVendor($vendor_id, Contract $contract) {
        ContractDetail::updateAll(['vendor_id' => $vendor_id], [
            'vendor_id' => $contract->vendor_id,
            'contract_id' => $contract->contract_id
        ]);

        //Actualizamos el vendedor del contrato
        $contract->updateAttributes(['vendor_id' => Yii::$app->request->post('Contract')['vendor_id']]);
    }
}
