...
class EmployeeBillMovement extends BaseMovement
{

    /**
     * @param $action string insert o update
     * @param $modelInstance EmployeeBill Instancia del modelo
            ...
            if ($modelInstance->status == "closed") {
                ...
                if ($action == "update") {
                    ...
                    foreach ($amounts as $key => $amount) {
                        ...
                        if ($key == 'total') {
                            if ($modelInstance->employee->account) {
                                $account_id = $modelInstance->employee->account->account_id;
                            } else {
                    ...
                    if( !( $account_movement_id = $countMov->createMovement(
                            Yii::t('app', 'Bill') . " - " . (trim($modelInstance->description) != '' ? $modelInstance->description . ' - ' : '') . $modelInstance->employee->fullName . ' .Fecha del comprobante: ' .$modelInstance->date,
