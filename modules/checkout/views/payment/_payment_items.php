...
$payment_method_cash = Config::getValue('payment_method_cash');
    ...
    $form = ActiveForm::begin([
        'id'=>'payment-item',
        'action' => ['add-item', 'id' => $model->payment_id],
        'options' => ['data-pjax' => true, 'onsubmit'=>'return false;' ]]); ?>
    <input type="hidden" name="PaymentItem[payment_id]" value="<?=$model->payment_id?>"/>
    ...
    <div class="row">
        <div class="col-sm-9 col-md-3">
            <div class="form-group">
                ...
                ?>
                <select name="PaymentItem[payment_method_id]" id="payment_method_id" class="form-control">
                    <?php
        ...
        <?php if (Yii::$app->getModule('paycheck') ) { ?>
            <div class="col-sm-9 col-md-9">
                <?= $this->render('@app/modules/paycheck/views/paycheck/_paycheck-selector', ['model' => $item, 'for_payment' => false]); ?>
            </div>
        <?php } ?>
    ...
    <div class="row">
        ...
        <div class="col-sm-9 col-md-3 field-paymentitem-amount">
            <?= $form->field($item, 'amount')->textInput(['id'=>'item_amount', 'class' => 'field-paymentitem-amount form-control'])  ?>
        </div>
...
<script>
    var PaymentItem = new function(){
        this.init = function() {
            $(document)
                .off("change", "#payment_method_id")
                .on("change", "#payment_method_id", function(){
                $("#money_box_id_small,#money_box_account_id_small,#paymentitem-number,#paycheck_search").val("");
                $("").val("");
...
<?php $this->registerJs('PaymentItem.init();'); ?>