...
class ContractController extends Controller {
    ...
    public function actionUpdate($id) {
        ...
        if($model->canUpdate()){
            ...
            $old_detail = $model->getLastContractDetailByType('plan');
            ...
            if ((Yii::$app->request->isPost && count($model->contractDetails) > 1 && User::hasRole('seller')) ||
                ...
                try {
                    $new_detail = new ContractDetail();
                    $new_detail->load(Yii::$app->request->post());

                    //Comparo para saber si los valores del detalle han cambiado
                    if(!$old_detail->isEqual($new_detail)) {
                        if($new_detail->from_date) {
                            if ((new DateTime($new_detail->from_date))->getTimestamp() >= (new DateTime($old_detail->from_date))->getTimestamp()) {
                                //Le pongo la fecha de fin al plan anterior para que la guarde en el log.
                                $old_detail->updateAttributes(['to_date' => (new DateTime($new_detail->from_date))->modify('-1 day')->format('Y-m-d')]);
                                $old_detail->createLog();
                                $customerLog = new CustomerLog();
                                $customerLog->createUpdateLog($model->customer_id, 'Plan', $old_detail->product->name, $old_detail->product->name, 'Contract', $model->contract_id);
                            } else {
                                $old_detail->addError('from_date', Yii::t('app', 'The new date cannot be greater than the old date.'));
                                throw new \Exception(Yii::t('app', 'The new date cannot be greater than the old date.'));
                            }

                            $model->addContractDetail(Yii::$app->request->post('ContractDetail'));
                            $model->cancelContractDetailsOutOfDateRange();
                        } else {
                            $old_detail->addError('from_date', Yii::t('app', 'The date cant be empty.'));
                        ...
                        if (User::hasPermission('user-can-select-vendor') && !empty(Yii::$app->request->post('Contract')['vendor_id'])) {
                            ...
                            ContractDetail::updateDetaillVendorAndContractVendor(Yii::$app->request->post('Contract')['vendor_id'], $model);
                        }

                        $model->updateAttributes(['instalation_schedule' => Yii::$app->request->post()['Contract']['instalation_schedule'],'address_id' => $address->address_id]);
                ...
                if(!$model->address_id) {
                    $model->updateAttributes(['address_id' => $customer->address_id]);
            ...
            $plans = Product::getPlansBySellerAndCustomer($customer, Yii::$app->user->identity->hasRole('seller', false));
            $vendors = Vendor::getVendorsWithActiveUser();

            return $this->render('update', [
                            'model' => $model,
                            'customer' => $customer,
                            'address' => $address,
                            'contractDetailPlan' => $old_detail,
                            'same_address' => $same_address,
                            'plans' => $plans,
                            'vendors' => $vendors
            ]);
