...
use app\modules\sale\models\search\CustomerSearch as SaleCustomerSearch;
...
class ReportsController extends Controller
{
    ...
    */
    ...
    public function actionCustomerDebt(){
        $this->layout = '//fluid';
        $searchModel = new SaleCustomerSearch;
        // $dataProvider = $searchModel->searchDebtors(Yii::$app->request->getQueryParams());
        $dataProvider = $searchModel->searchDebtorsV2(Yii::$app->request->getQueryParams());

        $dataProvider->setSort([
                'attributes' => ['code', 'name', 'currency','debt_bills'],
                'defaultOrder' => ['debt_bills' => SORT_DESC, 'currency' => SORT_DESC, 'name' => SORT_ASC],
        ]);
        // var_dump($dataProvider->getSort()->attributes); // to see the attrs set to sort
        ...
        // $customers = array(142073, 151864, 123723, 121099, 79336, 45815, 44264, 12431, 17823, 112662, 84386, 81201, 72937, 73292, 112562, 93671, 123457, 106691, 80749, 58392, 143941, 119285, 96375, 108628, 147042, 28002, 11910, 121160, 20140, 157031, 161348, 160990, 95116, 88156, 13693, 77486, 118457, 23418, 152559, 151642, 81246, 150507, 65306, 28685, 128430, 56390, 122867, 93879, 85711, 139014, 167014, 88436, 83875, 60699, 121184, 147386, 36451, 24469, 150834, 160825, 12409, 12562, 100176, 91653, 68657, 74787, 120270, 81436, 78668, 147758, 71694, 144075, 65025, 151689, 121453, 119184, 83827, 32803, 29010, 11738, 111572, 38951, 89021, 76732, 57437, 78876, 94845, 143592, 145879, 154810, 66481, 9468, 53130, 44594, 45534, 26664, 168180, 56377, 88328, 134498, 125110, 124804, 159728, 53536, 161677, 66117, 108167, 67457, 149965, 41701, 25986, 12910, 157151, 157030, 111843, 124576, 41532, 85046, 130813, 5542, 119473, 155399, 7361, 49733, 103622, 77480, 113359, 119133, 70178, 147369, 58869, 152749, 10475, 121141, 65949, 75728, 25645, 105842, 123526, 89781, 98505, 111250, 70913, 113088, 128383, 129721, 110214, 91433, 63918, 108759, 158791, 125433, 106682, 78407, 142080, 135717, 46734, 85295, 119157, 79687, 87638, 55451, 6157, 58905, 144048, 81756, 96499, 151866, 165287, 160994, 151793, 30942, 156596, 100004, 21822, 120688, 141610, 157610, 127952, 104854, 78404, 77758, 61566, 60134, 155390, 161683, 104043, 170197, 141332, 67316, 112111, 124736, 158160, 136128, 45359, 153961, 159723, 142364, 55891, 21858, 164216, 145265, 128536, 166766, 102870, 28929, 24594, 107247, 72718, 156307, 125954, 128350, 164798, 150836, 161792, 108108, 33701, 85140, 113097, 135725, 131964, 54762, 77283);
        $customers = array(142073, 151864, 123723, 121099, 79336, 45815, 44264);
